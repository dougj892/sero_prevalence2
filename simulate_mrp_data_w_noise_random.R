# The simulate_mrp_data function below is identical to the 
# function of the same name from the MRP with rstanarm vignette with one minor change.
# I added one lin to scale the overall prevalence in the data to the value p
...
simulate_mrp_data <- function(n) {
  ...
  coef_male <- rnorm(2,0, 2)
  coef_eth <- rnorm(3, 0,2)
  coef_age <- rnorm(7, 0,2)
  coef_income <- rnorm(3, 0,2)
  coef_state <- rnorm(50, 0,2)
  ...
  ##### ADDING LINE TO RESCALE CAT_PREF SO THAT MEAN IS p ####
  # ptrue_popn$cat_pref <- true_popn$cat_pref*(.1/mean(true_popn$cat_pref))
