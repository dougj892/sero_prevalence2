...
This notebook compares the performance of two different approaches to estimating seroprevalance given data from an imperfect test administered to a convenience sample. In the first approach, I break up the data into strata, calculate true prevalence and a confidence interval for the true prevalence use the epi.R package. I then calculate the variance of the estimate for each stratum by using the confidence interval and assuming normality. Finally, I use the standard formualas for combing estimates from different strata from stratified sampling.

In the second approach, I use a modified version of multi-level regression and poststratification. 
See the blog post [here]() for more information on the two approaches.

I compare these two approaches using a Monte Carlo simulation  Nearly all of the code to generate the simulated data is from the R vignette [MRP with rstanarm](https://cran.r-project.org/web/packages/rstanarm/vignettes/mrp.html) by Lauren Kennedy and Jonah Gabry. (Reproduced with their permission.)  Much of the Stan MRP code borrows from the code in the user guide.
...
write_csv(mc_results, file.path(path, "mcr1.csv"))
}
```
